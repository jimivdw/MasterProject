...
Following the current trend in web applications, \oframp{} has been implemented using the latest techniques from \verb|HTML5|, \verb|CSS3| and \verb|JavaScript|. It has turned out to be quite a big system, even though it consists of only three pages~(the application itself and the help and download pages). Combined with its three \verb|CSS| documents and 31 \verb|JavaScript| files, it consists of over 11,000 lines of code\footnote{This includes blank lines and lines containing just a closing bracket.}, or over 17,000 lines when the ten used - and often modified - external libraries are counted as well.
