...
The average \verb|SUS| / \verb|UMUX| results of the naive and smart versions of the system are shown in \figref{graph_sus_1}. As can be seen here, the naive version, with a median rating of $76$, scores better than the smart version, which has a median rating of only $61$. This means that the naive version scores above the average \verb|SUS| score of 68~\cite{}, where the smart version scores below that. In the adjective rating scale, the naive version is `good', but the smart version still scores `OK'~\cite{}.
