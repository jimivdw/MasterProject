...
Apart from the results we have already discussed in \chref{results}, some other results from \appref{graphs} are interesting to discuss. We provide an analysis of these results in this chapter.
...
However, as one can see almost immediately, there does not appear to be any correlation in any of the graphs shown in \figref{graph_correlation}. Even when the most extreme outliers are removed, the data points are still widely scattered and no correlation can be observed.
...
Just like is the case for the charge differences, the extremes in the number of used fragments are among the best results~(see \figref{graph_correlation_3}). However, it may again be the case that these points are just outliers. More experimentation is needed to be able to observe any actual trend.
