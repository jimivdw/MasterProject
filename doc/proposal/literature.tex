...
Nevertheless, being a closed standard with only limited possibilities, attempts have been made to replace the SMILES format. In 2005, a new format called IUPAC InChI was introduced~\cite{heller2013inchi}. InChI is an open format that aims to combine chemical, biological and other related information in a single representation. Still being a linguistic representation like SMILES, it is easily exchangeable and allows for simple machine processing. In the past few years, InChI has become the international, worldwide standard for defined chemical structures. As SMILES is also still widely used, the tool that will be developed in this research project will support both the SMILES and InChI formats for entering molecules into the system.
