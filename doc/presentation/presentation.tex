...
Quick recap:
\begin{itemize}
\item Two experiment groups;
\item One group did consuming first, other started with producing;
\item Two molecules per version.
\end{itemize}
\end{frame}
...
\begin{frame}{x}{}
 \begin{columns}
  \column{.45\textwidth}
   \begin{itemize}
    \item \emph{All} users preferred consuming version;
    \item Interface very clear and responsive;
    \item Easy to learn;
    \item<5-> Finished popup confusing;
    \item<5-> Fragment matching not perfect;
    \item<5-> Charge edit missing.
   \end{itemize}

  \column{.45\textwidth}
   \begin{itemize}
    \item<2-> ``Excellent job, quality and functionality!''
    \item<3-> ``Nice work!''
    \item<4-> ``I think that the system has great potential!''
   \end{itemize}
